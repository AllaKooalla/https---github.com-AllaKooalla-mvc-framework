...
class Product extends AppModel
{
    ...
    public function product_validate(): bool
    {
        ...
        foreach ($_POST['product_description'] as $item) {
            ...
            if (empty($item['exerpt'])) {
                $errors .= "Не заполнено Краткое описание <br>";
            }
        }
    ...
    public function save_product(): bool
    {
        ...
        try {
            ...
            $product->slug = AppModel::create_slug('product', 'slug', $_POST['product_description']['1']['title'], $product_id);
            R::store($product);
    ...
    {
        return R::getRow("SELECT pd.download_id, dd.name FROM product_download pd JOIN download_description dd ON pd.download_id = dd.download_id WHERE pd.product_id = ? AND dd.language_id = 1", [$product_id]);
    }

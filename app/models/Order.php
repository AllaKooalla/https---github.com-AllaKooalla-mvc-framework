...
class Order extends AppModel
{
    ...
    public static function saveOrder($data): int|false
    {
        ...
        try {
            ...
            // распечатать ошибки, если они есть
    ...
    public static function mailOrder($order_id, $user_email, $tpl): bool
    {
        ...
        try {
            ...
            $mail->SMTPDebug = 3;
            $mail->CharSet = 'UTF-8';
            $mail->Host = App::$app->getProrerty('smtp_host');
            $mail->SMTPAuth = App::$app->getProrerty('smtp_auth');
            $mail->Username = App::$app->getProrerty('smtp_username');
            $mail->Password = App::$app->getProrerty('smtp_password');
            $mail->SMTPSecure = App::$app->getProrerty('smtp_secure');
            $mail->Port = App::$app->getProrerty('smtp_port');
            $mail->isHTML(true);

            $mail->setFrom(App::$app->getProrerty('smtp_from_email'), App::$app->getProrerty('site_name'));
            $mail->addAddress($user_email);
            $mail->Subject = sprintf("Заказ N%d", $order_id);
